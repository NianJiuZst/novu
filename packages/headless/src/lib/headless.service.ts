import { ApiService, IStoreQuery, IUserGlobalPreferenceSettings, IUserPreferenceSettings } from '@novu/client';
import { IMessage, INotificationDto, IOrganizationEntity, IPaginatedResponse, WebSocketEventEnum } from '@novu/shared';
import { MutationObserverResult, QueryClient, QueryObserverOptions, QueryObserverResult } from '@tanstack/query-core';
...
export class HeadlessService {
  ...
  private organizationQueryOptions: QueryObserverOptions<IOrganizationEntity, unknown> = {
  ...
  private unseenCountQueryOptions: QueryObserverOptions<{ count: number }, unknown> = {
    queryKey: UNSEEN_COUNT_QUERY_KEY,
    queryFn: () => this.api.getUnseenCount(),
  };

  private unreadCountQueryOptions: QueryObserverOptions<{ count: number }, unknown> = {
    queryKey: UNREAD_COUNT_QUERY_KEY,
    queryFn: () => this.api.getUnreadCount(),
  };

  private userPreferencesQueryOptions: QueryObserverOptions<IUserPreferenceSettings[], unknown> = {
    queryKey: USER_PREFERENCES_QUERY_KEY,
    queryFn: () => this.api.getUserPreference(),
  };

  private userGlobalPreferencesQueryOptions: QueryObserverOptions<IUserGlobalPreferenceSettings[], unknown> = {
  ...
  private applyToken(newToken: string | null) {
    if (newToken) {
      isBrowser && localStorage.setItem(NOTIFICATION_CENTER_TOKEN_KEY, newToken);
  ...
  private callFetchListener = <T>(result: QueryObserverResult<T>, listener: (result: FetchResult<T>) => void) =>
    listener({
      data: result.data,
      error: result.error,
  ...
  private callUpdateListener = <TData = unknown, TError = unknown, TVariables = unknown>(
  ...
  public listenNotificationReceive({ listener }: { listener: (message: IMessage) => void }) {
    this.assertSessionInitialized();

    if (this.socket) {
      this.socket.on(WebSocketEventEnum.RECEIVED, (data?: { message: IMessage }) => {
        if (data?.message) {
  ...
  public listenUnseenCountChange({ listener }: { listener: (unseenCount: number) => void }) {
    this.assertSessionInitialized();

    if (this.socket) {
      this.socket.on(WebSocketEventEnum.UNSEEN, (data?: { unseenCount: number }) => {
        if (Number.isInteger(data?.unseenCount)) {
          this.queryClient.setQueryData<{ count: number }>(UNSEEN_COUNT_QUERY_KEY, (oldData) => ({
  ...
  public listenUnreadCountChange({ listener }: { listener: (unreadCount: number) => void }) {
    this.assertSessionInitialized();

    if (this.socket) {
      this.socket.on(WebSocketEventEnum.UNREAD, (data?: { unreadCount: number }) => {
        if (Number.isInteger(data?.unreadCount)) {
          this.queryClient.setQueryData<{ count: number }>(UNREAD_COUNT_QUERY_KEY, (oldData) => ({
    ...
    >({
      options: {
        mutationFn: (variables) =>
          this.api.updateSubscriberPreference(variables.templateId, variables.channelType, variables.checked),
        onSuccess: (data) => {
          this.queryClient.setQueryData<IUserPreferenceSettings[]>(USER_PREFERENCES_QUERY_KEY, (oldUserPreferences) =>
  ...
  public async updateUserGlobalPreferences({
    ...
    listener: (
      result: UpdateResult<IUserGlobalPreferenceSettings, unknown, IUpdateUserGlobalPreferencesVariables>
    ) => void;
    onSuccess?: (settings: IUserGlobalPreferenceSettings) => void;
    onError?: (error: unknown) => void;
  }) {
    ...
    >({
      options: {
        ...
        onSuccess: (data) => {
          this.queryClient.setQueryData<IUserGlobalPreferenceSettings[]>(USER_GLOBAL_PREFERENCES_QUERY_KEY, () => [
            data,
          ]);
        },
      },
  ...
  public async markNotificationsAsRead({
    ...
    const { result, unsubscribe } = this.queryService.subscribeMutation<IMessage[], unknown, { messageId: IMessageId }>(
      {
        options: {
          mutationFn: (variables) =>
  ...
  public async removeNotification({
    ...
    const { result, unsubscribe } = this.queryService.subscribeMutation<IMessage, unknown, { messageId: string }>({
      options: {
  ...
  public async removeNotifications({
    ...
    messageIds: string[];
    listener: (result: UpdateResult<void, unknown, { messageIds: string[] }>) => void;
    onSuccess?: (obj: void) => void;
    onError?: (error: unknown) => void;
  }) {
    this.assertSessionInitialized();

    const { result, unsubscribe } = this.queryService.subscribeMutation<void, unknown, { messageIds: string[] }>({
      options: {
  ...
  public async updateAction({
    ...
    const { result, unsubscribe } = this.queryService.subscribeMutation<IMessage, unknown, IUpdateActionVariables>({
      options: {
  ...
  public async markAllMessagesAsRead({
    ...
    const { result, unsubscribe } = this.queryService.subscribeMutation<number, unknown, { feedId?: IFeedId }>({
      options: {
        mutationFn: (variables) => this.api.markAllMessagesAsRead(variables?.feedId),
        onSuccess: (data) => {
          this.queryClient.refetchQueries(NOTIFICATIONS_QUERY_KEY, {
            exact: false,
          });
  ...
  public async markAllMessagesAsSeen({
    ...
    const { result, unsubscribe } = this.queryService.subscribeMutation<number, unknown, { feedId?: IFeedId }>({
      options: {
        mutationFn: (variables) => this.api.markAllMessagesAsSeen(variables?.feedId),
        onSuccess: (data) => {
          this.queryClient.refetchQueries(NOTIFICATIONS_QUERY_KEY, {
            exact: false,
          });
  ...
  public async removeAllNotifications({
    ...
    const { result, unsubscribe } = this.queryService.subscribeMutation<void, unknown, { feedId?: string }>({
      options: {
        mutationFn: (variables) => this.api.removeAllMessages(variables?.feedId),
        onSuccess: (data) => {
