...
import type { SubscribersControllerSearchSubscribersRequest } from '@novu/api/src/models/operations';
  ...
  describe('Create Subscriber', () => {
    it.skip(`should not create multiple subscribers when multiple triggers are made        
      ...
      for (let i = 0; i < 2; i += 1) {
        ...
        await Promise.all([
          novuClient.subscribers.create({
            subscriberId,
            firstName: 'TestSubFName',
            lastName: 'TestSubLName',
          }),
          novuClient.subscribers.create({
            subscriberId,
            firstName: 'TestSubFName',
