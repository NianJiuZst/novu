...
import { ConditionType, SubscriberEntity, TopicRepository, TopicSubscribersRepository } from '@novu/dal';
    ...
    try {
      ...
      for (let i = 0; i < MAX_SUBSCRIPTIONS_PER_SUBSCRIBER; i++) {
        const response = await novuClient.topics.subscriptions.create(
          {
            subscriberIds: [subscriber1.subscriberId],
            rules: [{ filter: { workflows: [`workflow-${i}`] }, type: ConditionType.CHECKBOX, condition: true }],
          },
          topicKey
        );
