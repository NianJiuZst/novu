...
describe('Upsert Layout #novu-v2', () => {
  ...
  describe('Create Layout - POST /v2/layouts', () => {
    ...
    it('should preserve custom identifier when provided and not use autogenerated identifier', async () => {
      const customLayoutId = 'my-custom-layout-identifier';
      const layoutName = 'Layout Name With Spaces And Dashes';
      ...
      expect(createdLayout).to.exist;
      expect(createdLayout.layoutId).to.equal(customLayoutId);
      expect(createdLayout.name).to.equal(layoutName);
      // The identifier should NOT be slugified from the name, but should be the custom layoutId
      expect(createdLayout.layoutId).to.not.equal('layout-name-with-spaces-and-dashes');
    });
