...
export function WorkflowTemplateModal(props: WorkflowTemplateModalProps) {
  ...
  const filteredTemplates = WORKFLOW_TEMPLATES.filter((template) => {
    ...
    if (selectedCategory === 'clerk') {
      return template.workflowDefinition.tags?.includes('clerk');
    }
  ...
  const getHeaderText = () => {
    ...
    if (mode === WorkflowMode.TEMPLATES) {
      // Check if we have a special integration config for this category
      if (selectedCategory in integrationConfigs) {
        const config = integrationConfigs[selectedCategory];
        return (
          <div className="flex items-center gap-1.5">
            <span>{config.parentCategory} (with</span>
            <div className="flex items-center">{config.icon}</div>
            <span>{config.postfix})</span>
          </div>
        );
      }
  ...
  const handleImportCollection = async () => {
    if (selectedCategory === 'stripe' || selectedCategory === 'clerk') {
      ...
      const templatesToImport = WORKFLOW_TEMPLATES.filter((template) =>
        template.workflowDefinition.tags?.includes(selectedCategory)
      );
  ...
  return (
    <Dialog open={props.open} onOpenChange={props.onOpenChange}>
      ...
      <DialogContent className="w-full max-w-[1240px] gap-0 p-0" id="workflow-templates-modal">
        ...
        {!selectedTemplate && (selectedCategory === 'stripe' || selectedCategory === 'clerk') && (
          <DialogFooter className="border-stroke-soft !mx-0 flex justify-between border-t !p-1.5">
            <div className="flex items-center">
              <span className="text-paragraph-sm mr-3 text-gray-500">
            ...
            >
              <div className="flex items-center gap-2">
                ...
                <span className="rounded-sm bg-white/20 px-1.5 py-0.5 text-xs">
                  {WORKFLOW_TEMPLATES.filter((t) => t.workflowDefinition.tags?.includes(selectedCategory)).length}
                </span>
              </div>
            </Button>
          </DialogFooter>
